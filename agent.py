...
<img id=1 Google/>
<input id=2 text Search Search/>
<button id=3 Search by voice/>
<button id=4 Search by image/>
<button id=5 Google Search/>
<button id=6 I'm Feeling Lucky/>
<text id=7>Google offered in:</text>
<link id=8>हिन्दी</link>
<link id=9>বাংলা</link>
<link id=10>తెలుగు</link>
<link id=11>मराठी</link>
<link id=12>தமிழ்</link>
<link id=13>ગુજરાતી</link>
<link id=14>ಕನ್ನಡ</link>
<link id=15>മലയാളം</link>
<link id=16>ਪੰਜਾਬੀ</link>
------------------
OBJECTIVE: Find a 2 bedroom house for sale in Anchorage AK for under $750k
CURRENT URL: https://www.www.google.com/
YOUR COMMAND: 
TYPESUBMIT 2 "anchorage redfin"
==================================================
...
CURRENT URL: https://www.www.google.com/
YOUR COMMAND: 
TYPESUBMIT 2 "dorsia nyc opentable"
==================================================
...
<link id=16>ਪੰਜਾਬੀ</link>
------------------
OBJECTIVE: buy a pizza from silchar
CURRENT URL: https://www.www.google.com/
YOUR COMMAND: 
TYPESUBMIT 2 "pizza hut Silchar"
==================================================
...
if (
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		...
		#send a request to the nodeJS server which will be running the openAI script
		response = requests.post(url='http://localhost:2003/get-gpt-command', json=data).text
	...
	def run_cmd(cmd):
		...
		if cmd.startswith("SCROLL UP"):
			...
			#a special condition for the google webpage
			if cmd.startswith("TYPESUBMIT 2 "):	
				search_query = cmd.removeprefix("TYPESUBMIT 2 ").replace(" ", "+")
				print("generated search query: " + search_query)
				_crawler.go_to_page("www.google.com/search?q=" + search_query)				
			if cmd.startswith("TYPESUBMIT 4 "):	
				search_query = cmd.removeprefix("TYPESUBMIT 4 ").replace(" ", "+")
				print("generated search query: " + search_query)
				_crawler.go_to_page("www.google.com/search?q=" + search_query)	
	...
	gpt_cmd = ""
	prev_cmd = ""
	_crawler.go_to_page("www.google.com")
	try:
		while True:
			browser_content = "\n".join(_crawler.crawl())
			prev_cmd = gpt_cmd
			...
			if command == "r" or command == "":
				...
				#we first move to the google webpage before issuing any search query
				_crawler.go_to_page("www.google.com")
